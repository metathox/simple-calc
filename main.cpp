...
double Calculator::applyOperation(double x, double y, char op)
{
    switch(op)
    {
        ...
        case '*': return x * y;
        case '/': 
            if(y == 0) { std::cerr << "Division by zero!\n"; return 0; }
            return x / y;
        case '^': return pow(x, y);
...
std::vector<Calculator::Token> Calculator::tokenize()
{
    ...
    while(i < expr.size())
    {
        ...
        if(isdigit(c) || (c == '.' && i + 1 < expr.size() && isdigit(expr[i + 1])))
        {
            ...
            --i;  // adjust index since we overshoot in while loop
...
std::vector<Calculator::Token> Calculator::toPostfix(const std::vector<Token>& tokens)
{
    ...
    auto isRightAssociative = [](char op){ return op == '^' || op == 'u'; };
    ...
    for(const auto &tok : tokens)
    {
        ...
        else if(tok.type == Token::PAREN_RIGHT)
        {
            ...
            if(!opStack.empty() && opStack.top().type == Token::PAREN_LEFT) opStack.pop();
        }
    }
